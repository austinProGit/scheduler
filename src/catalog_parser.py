...
def extract_prerequisites():
    ...
    _prerequisites = ''  # create blank string for prerequisites
    try:  # course_body list varies in size from 1 to 3
        course_pattern = r'[A-Z]{4}\s{1}\d{4}'  # pattern for all courses
        print(course_body[1].text)
        # sometimes the hex value, \xa0, is in between course id's. Ex: MATH\xa01113
        _prerequisites = re.findall(course_pattern, str(course_body[1].text).replace("\xa0", " "))
        ...
        bad_courses = ['CSCI 1301', 'CYBR 2106', 'CPSC 5115']  # typo's in data, or out of date
        ...
        print(_prerequisites)
    except IndexError:  # course has no prerequisites or readability, only description
        pass
...
course_info_df = pd.DataFrame(columns=['ID', 'name', 'hours', 'availability', 'prerequisites', 'co-requisites',
                                       'recommended', 'isElective', 'restrictions', 'note'])
...
# export to excel, and save to specific sheets TODO: create and add CYBR sheet
excel = pd.ExcelWriter('Course Info.xlsx')
course_info_df.to_excel(excel, 'CPSC')
excel.save()
