...
def degreeworks_parser_tests():
    ...
    for test in test_case_list:
        ...
        # print(f'expected output: {test["ex_out"]}')
        print(f'generate_degree_extraction_container(test["test_data"])[0]: {generate_degree_extraction_container(test["test_data"])[0]}')
        print('\n')
        print(f'test["ex_out"]: {test["ex_out"]}')
        if generate_degree_extraction_container(test["test_data"])[0].replace(" ", "").replace("\t", "").replace("\n","") == test["ex_out"].replace(" ", "").replace("\t", "").replace("\n",""):
            test_passed = True
